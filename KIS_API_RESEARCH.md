...
### 방안 B — WebSocket 실시간 (채택)
- 백엔드에서 KIS WebSocket 연결 유지
- 거래량 TOP 5 종목 자동 구독 → 메모리 캐시 갱신 → 프론트에 브로드캐스트
- 프론트는 Spring WebSocket(`/ws/stock`)으로 실시간 수신

### 최종 설계 (2026-03-18 확정)
- **목적**: 매수/매도 없이 실시간 시세 조회 전용 (실전투자 계좌 발급, 수수료 없음)
- **구독 대상**: 서버 시작 시 DB 거래량 TOP 5 종목 자동 구독 (고정)
- **장 중/마감 처리**: 09:00 구독 시작, 15:30 이후 구독 해제 (불필요한 WS 유지 방지)
- **캐시**: 최신 가격을 메모리에 보관 → 신규 프론트 접속 시 즉시 전송
- **프론트 표시**: StockView 거래량 TOP 리스트에 장 중이면 실시간 가격, 마감 후면 전일 종가 표시
