...
코드 작업과 함께 **항상** 아래 MD 파일들을 최신 상태로 유지한다.

### CHANGELOG.md
- 코드 변경이 있을 때마다 최상단에 날짜별 섹션 추가
- 형식: `## YYYY-MM-DD` → 변경 내용 bullet point

### STOCK_CHART_TODO.md (TODO 리스트)
- 작업을 시작할 때: `[ ]` 항목 확인
- 작업을 완료할 때: `[ ]` → `[x]` 로 변경, 완료 날짜 추가
- 새로운 할 일이 생기면: 적절한 섹션에 `[ ]` 항목 추가

### INCIDENT_2026-03-14.md (이슈 보고서)
- 새로운 버그/장애 발생 시: 상단에 🔴 또는 🟡 섹션 추가
- 이슈 해결 시: 해결 방법(어떻게 고쳤나) 항목 작성
- 재발 방지책 추가

### 규칙 요약
```
코드 변경 → CHANGELOG.md 업데이트
이슈 발생 → INCIDENT md에 추가
이슈 해결 → INCIDENT md에 해결 방법 추가
작업 완료 → TODO md에서 [x] 처리
새 작업   → TODO md에 [ ] 추가
```
